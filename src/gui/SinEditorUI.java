...
public class SinEditorUI extends JDialog {
	...
	public SinEditorUI() {
				...
				FormFactory.DEFAULT_ROWSPEC,}));
		{
			...
			contentPanel.add(lblSinFil, "2, 2");
		}
		{
			textSin = new JTextField();
			contentPanel.add(textSin, "2, 4, fill, default");
		...
		}
		{
			...
			contentPanel.add(button, "4, 4");
		}
		{
			JLabel lblPartitionInfo = new JLabel("Partition Info :");
			contentPanel.add(lblPartitionInfo, "2, 6");
		}
		{
			textPartition = new JTextField();
			contentPanel.add(textPartition, "2, 8, fill, default");
			textPartition.setColumns(10);
		}
		{
			JLabel lblSpareInfo = new JLabel("Spare Info :");
			contentPanel.add(lblSpareInfo, "2, 10");
		}
		{
			textSpare = new JTextField();
			contentPanel.add(textSpare, "2, 12, fill, default");
		...
		}
		{
			...
			btnDumpData.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					try {
						sin.dumpImage();
						MyLogger.getLogger().info("Dump finished to "+sin.getImage());
					}
					catch (Exception e) {
